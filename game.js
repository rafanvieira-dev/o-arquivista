...
function gameLoop(timeStamp) {
    ...
    if (gameState === 'PLAYING') {
        ...
        levelData.flyingEnemies.forEach(e => e.update(deltaTime));
        ...
        // MECÂNICA DE RATOS INVENCÍVEIS (NO CHÃO)
        ...
        // MECÂNICA DE BARATAS VOADORAS (NO AR)
        levelData.flyingEnemies.forEach(flyingEnemy => {
            if (isColliding(player, flyingEnemy)) {
                if (!player.invincible) {
                    health--; updateHUD(); 
                    player.invincible = true;
                    setTimeout(() => player.invincible = false, 1500); 
        ...
        levelData.flyingEnemies.forEach(e => e.draw(ctx, cameraX));
